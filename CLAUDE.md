...
- **lumen-cli** — Clap-based CLI (`main.rs`, ~210 lines) orchestrating compiler → VM
...
4. **Resolution** (`compiler/resolve.rs`) — Builds symbol table, infers effects, emits effect diagnostics
...
The VM (`vm/vm.rs`) is a register-based interpreter with a call-frame stack (max depth 256). Runtime values include scalars, collections, records, unions, closures, futures, and trace refs. Process runtimes (memory, machine) provide instance-scoped state.
...
- **Processes** (memory, machine, pipeline, etc.) are constructor-backed runtime objects
