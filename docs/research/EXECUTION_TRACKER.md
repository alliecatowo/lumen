...
| Ecosystem | Lockfile/frozen is path-focused; registry search/upload still stubbed. | Lockfile v2 schema + compatibility reader drafted and tested. | Registry MVP: `pkg search` + upload path (replacing dry-run-only publish) behind explicit env/config. | Integration test proves publish/search/install round-trip on local registry fixture. |
...
3. By 2026-03-13: Implement registry-backed `pkg search` and non-dry-run publish upload path.
4. By 2026-03-15: Lockfile v2 format migration tested (read v1 + write v2 deterministically).
