...
fn intrinsic_description(id: &IntrinsicId) -> &str {
    ...
    match id {
        ...
        ReadLines => "Read a file and return lines as a list of strings",
        WalkDir => "Recursively list all file paths in a directory",
        GlobMatch => "Find files matching a glob pattern",
        PathJoin => "Join two path components",
        PathParent => "Return the parent directory of a path",
        PathExtension => "Return the file extension of a path",
        PathFilename => "Return the filename component of a path",
        PathStem => "Return the filename without extension",
        Exec => "Execute a shell command and return stdout, stderr, and status",
        ReadStdin => "Read all of stdin to a string",
        Eprint => "Print to stderr without a newline",
        Eprintln => "Print to stderr with a newline",
        CsvParse => "Parse a CSV string into a list of lists of strings",
        CsvEncode => "Encode a list of lists into a CSV string",
        TomlParse => "Parse a TOML string into a map",
        TomlEncode => "Encode a map into a TOML string",
        RegexMatch => "Return capture groups for the first regex match",
        RegexReplace => "Replace all regex matches in a string",
