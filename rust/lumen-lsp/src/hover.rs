...
fn get_builtin_hover(name: &str) -> Option<Hover> {
    let builtin_docs = vec![
        ...
        ("append", "append(list, item) -> list", "Append an item to a list"),
        ("sort", "sort(list) -> list", "Sort a list"),
        ("map", "map(list, fn) -> list", "Transform each element"),
        ("filter", "filter(list, fn) -> list", "Filter by predicate"),
        ("reduce", "reduce(list, init, fn) -> value", "Reduce to single value"),
        ("join", "join(list, separator) -> String", "Join to string"),
        ("split", "split(string, separator) -> list[String]", "Split string"),
        ("trim", "trim(string) -> String", "Trim whitespace"),
        ("parse_int", "parse_int(string) -> result[Int, String]", "Parse to int"),
        ("parse_float", "parse_float(string) -> result[Float, String]", "Parse to float"),
        ("to_string", "to_string(value) -> String", "Convert to string"),
        ("contains", "contains(collection, item) -> Bool", "Check membership"),
        ...
        ("vote", "vote(futures, threshold) -> value", "Vote on results"),
